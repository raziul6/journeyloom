...
<div class="wrap wptm-admin-wrap">
    <div class="wptm-admin-header">
        ...
        <div>
            <h1><?php esc_html_e( 'Search Form Builder', 'journeyloom' ); ?></h1>
            <p style="margin:4px 0 0;opacity:.8;font-size:13px;"><?php esc_html_e( 'Toggle fields on or off, rename them, and drag to reorder. Changes apply to the live search form.', 'journeyloom' ); ?></p>
        </div>
        <span class="wptm-version"><?php echo esc_html( count( array_filter( $fields, function ( $f ) { return ! empty( $f['enabled'] ); } ) ) ); ?> <?php esc_html_e( 'active', 'journeyloom' ); ?></span>
    </div>
    ...
    <form id="wptm-search-form-builder">
        ...
        <div class="wptm-sfb-grid" id="wptm-sortable-fields">
            ...
            <div class="wptm-sfb-card<?php echo empty( $field['enabled'] ) ? ' is-off' : ''; ?>" data-field="<?php echo esc_attr( $key ); ?>" draggable="false">
                <div class="wptm-sfb-card__head">
                    <span class="wptm-sfb-drag dashicons dashicons-menu" title="<?php esc_attr_e( 'Drag to reorder', 'journeyloom' ); ?>"></span>
                    ...
                    <label class="wptm-switch" title="<?php esc_attr_e( 'Enable field', 'journeyloom' ); ?>">
        ...
        <div class="wptm-sfb-footer">
            <p class="wptm-sfb-shortcode">
                ...
                <?php esc_html_e( 'Embed anywhere:', 'journeyloom' ); ?> <code>[wptm_search_form]</code>
            </p>
            <button type="submit" class="button button-primary button-hero wptm-sfb-save"><?php esc_html_e( 'Save Search Form', 'journeyloom' ); ?></button>
        </div>
    </form>
</div>
